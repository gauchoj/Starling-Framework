package starling.core
{
	...
	import com.assukar.engine.utils.UncaughtErrorHandler;
...
//            mEnableErrorChecking = false;
        ...
        private function requestContext3D( stage3D:Stage3D, renderMode:String, profile:Object ):void
        {
            ...
            if (profile == "auto") profiles = ["standardExtended", "standard", "standardConstrained", "baselineExtended", "baseline", "baselineConstrained"];
            else if (profile is String) profiles = [profile as String];
            else if (profile is Array) profiles = profile as Array;
            else
            ...
            function requestNextProfile():void
            {
                ...
                try
                { execute(mStage3D.requestContext3D, renderMode, currentProfile); } catch (error:Error)
                {
...
//            Utils.printStackTrace("pendingInitialization=true initializations=" + initializations);
        ...
        public var enterFrameDate:Date;
        ...
        public function renderFrame():void
        {
            ...
            if (isStarted)
            {
                if (!UncaughtErrorHandler.ERROR)
                {
                    nextFrame();
                }
                else
                {
                    UncaughtErrorHandler.ERROR = false;
...
//                    Utils.log("CALLING FRAMECALLBACK " + i);
        ...
        private function onTouch( event:Event ):void
        {
            ...
            if (event.type == MouseEvent.MOUSE_UP && Mouse.supportsCursor) mTouchProcessor.enqueue(touchID, TouchPhase.HOVER, globalX, globalY);
        }
        ...
        private function get touchEventTypes():Array
        {
            ...
            if (multitouchEnabled) types.push(TouchEvent.TOUCH_BEGIN, TouchEvent.TOUCH_MOVE, TouchEvent.TOUCH_END);
            
            if (!multitouchEnabled || Mouse.supportsCursor) types.push(MouseEvent.MOUSE_DOWN, MouseEvent.MOUSE_MOVE, MouseEvent.MOUSE_UP);
...
//        }
        ...
        {
            if (sCurrent) throw new IllegalOperationError("'multitouchEnabled' must be set before Starling instance is created");
        ...
        {
            if (sCurrent) throw new IllegalOperationError("'handleLostContext' must be set before Starling instance is created");
            else
