...
package starling.display {
	 ...
	 *  @see Sprite
	 */
	public class QuadBatch_ extends DisplayObject {
		...
		private function createBuffers():void {
			...
			// double side			
			var mIndexDatadoubleSidedd:Vector.<uint>;			
			if (doubleSided) {						
				mIndexDatadoubleSidedd = new <uint>[];			
				var i:uint = 0, //
					index:uint = 0, //
					len:uint = mIndexData.length;
				for (; i < len; i++) {
					mIndexDatadoubleSidedd[index + 0] = mIndexData[i + 0];
					mIndexDatadoubleSidedd[index + 1] = mIndexData[i + 1];
					mIndexDatadoubleSidedd[index + 2] = mIndexData[i + 2];
					mIndexDatadoubleSidedd[index + 3] = mIndexData[i + 3];
					mIndexDatadoubleSidedd[index + 4] = mIndexData[i + 4];
					mIndexDatadoubleSidedd[index + 5] = mIndexData[i + 5];
					
					mIndexDatadoubleSidedd[index + 6] = mIndexData[i + 5];
					mIndexDatadoubleSidedd[index + 7] = mIndexData[i + 4];
				...
				mIndexData = mIndexDatadoubleSidedd;
			}
			
			var numVertices:int = mVertexData.numVertices;
			
			// double side
//			var numIndices:int = doubleSided ? mIndexDatadoubleSidedd.length : mIndexData.length;
			...
			// double side
//			mIndexBuffer.uploadFromVector(doubleSided ? mIndexDatadoubleSidedd : mIndexData, 0, numIndices );
		...
		public function addImage(image:Image, parentAlpha:Number = 1.0, modelViewMatrix:Matrix = null, blendMode:String = null):void {
			...
			// double side
			addQuad(image, parentAlpha, image.texture, image.smoothing, image.doubleSided, modelViewMatrix, blendMode);
//			addQuad(image, parentAlpha, image.texture, image.smoothing, modelViewMatrix, blendMode);
		...
		//double side
		public function addQuad(quad:Quad, parentAlpha:Number = 1.0, texture:Texture = null, smoothing:String = null, doubleSided:Boolean = false, modelViewMatrix:Matrix = null, blendMode:String = null):void {
...
//		public function addQuad(quad:Quad, parentAlpha:Number = 1.0, texture:Texture = null, smoothing:String = null, modelViewMatrix:Matrix = null, blendMode:String = null):void {
		...
		public function addQuadBatch(quadBatch:QuadBatch, parentAlpha:Number = 1.0, modelViewMatrix:Matrix = null, blendMode:String = null):void {
			...
			if (mNumQuads == 0) {
				...
				// double sided
				this.doubleSided = quadBatch.doubleSided;		
		...
		public function isStateChange(tinted:Boolean, parentAlpha:Number, texture:Texture, smoothing:String, doubleSided:Boolean, blendMode:String, numQuads:int = 1):Boolean {
//		public function isStateChange(tinted:Boolean, parentAlpha:Number, texture:Texture, smoothing:String, blendMode:String, numQuads:int = 1):Boolean {
			if (mNumQuads == 0)
				...
				return mTexture.base != texture.base || mTexture.repeat != texture.repeat || mSmoothing != smoothing || mTinted != (tinted || parentAlpha != 1.0) || this.blendMode != blendMode || this.doubleSided != doubleSided;
		...
		private static function compileObject(object:DisplayObject, quadBatches:Vector.<QuadBatch>, quadBatchID:int, transformationMatrix:Matrix, alpha:Number = 1.0, blendMode:String = null, ignoreCurrentFilter:Boolean = false):int {
			...
			if (filter && !ignoreCurrentFilter) {
				...
				if (quad) {
					...
					// double side
					doubleSided = image ? image.doubleSided : false;
				...
				// double side
				if (quadBatch.isStateChange(tinted, alpha * objectAlpha, texture, smoothing, doubleSided ,blendMode, numQuads))
...
//				if (quadBatch.isStateChange(tinted, alpha * objectAlpha, texture, smoothing, blendMode, numQuads))
				...
				if (quad)
					...
					// double side
					quadBatch.addQuad(quad, alpha, texture, smoothing, doubleSided, transformationMatrix, blendMode);					
