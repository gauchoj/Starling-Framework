...
//			sNativeTextField.sharpness = -50;
			...
			if (hAlign == HAlign.LEFT)
				textOffsetX = 2; // flash adds a 2 pixel offset
			...
			if (vAlign == VAlign.TOP)
				textOffsetY = 2; // flash adds a 2 pixel offset
...
//            }else {
			//bitmapData.lock();
			//bitmapData.draw(sNativeTextField, drawMatrix, null, null, null, false);
			//bitmapData.unlock();
//			}
...
//			
		...
		private function calculateFilterOffset(textField:flash.text.TextField, hAlign:String, vAlign:String):Point {
			...
			if (filters != null && filters.length > 0) {
				...
				for each (var filter:BitmapFilter in filters) {
					...
					var filterBounds:Rectangle = new Rectangle(offsetX, offsetY, textWidth + marginX, textHeight + marginY);
		...
		private function get isHorizontalAutoSize():Boolean {
			return mAutoSize == TextFieldAutoSize.HORIZONTAL || mAutoSize == TextFieldAutoSize.BOTH_DIRECTIONS;
		}
		
		private function get isVerticalAutoSize():Boolean {
			return mAutoSize == TextFieldAutoSize.VERTICAL || mAutoSize == TextFieldAutoSize.BOTH_DIRECTIONS;
		}
