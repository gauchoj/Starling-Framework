...
package starling.textures
{
    ...
    public class RenderTexture extends SubTexture
    {
         ...
         */
        public function RenderTexture( name:String, width:int, height:int, persistent:Boolean = true, scale:Number = -1, format:String = "bgra", repeat:Boolean = false )
        {
//			this.name = name;
        ...
        public function draw( object:DisplayObject, matrix:Matrix = null, alpha:Number = 1.0, antiAliasing:int = 0 ):void
        {
            ...
            if (mDrawing) render(object, matrix, alpha);
            else renderBundled(render, object, matrix, alpha, antiAliasing);
        }
        ...
        private function renderBundled( renderBlock:Function, object:DisplayObject = null, matrix:Matrix = null, alpha:Number = 1.0, antiAliasing:int = 0 ):void
        {
            ...
            if (isDoubleBuffered || !isPersistent || !mBufferReady) mSupport.clear();
            
            // draw buffer
            if (isDoubleBuffered && mBufferReady) mHelperImage.render(mSupport, 1.0);
            else mBufferReady = true;
